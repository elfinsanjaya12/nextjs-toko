...
function Cart() {
  ...
  useEffect(() => {
    /** check cart local dengan yang ada di dalam database */
    ...
    if (cartLocal && cartLocal.length > 0) {
      ...
      const updateCart = async () => {
        for (const item of cartLocal) {
          ...
          // jika stock lebih dari 0 simpan ke cart
          if (inStock > 0) {
            newArr.push({
  ...
  if (cart.length === 0)
    return <img className="img-responsive w-100" src="/empty_cart.jpeg" alt="not empty" />
